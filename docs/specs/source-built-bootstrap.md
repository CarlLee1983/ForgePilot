...
Install and upgrade support Darwin arm64 only and require Git, `make`, and an already-installed Go toolchain compatible with the `go` directive at the exact source commit. They do not install Go, a package manager, dependencies, or a shell profile. Planning binds the resolved Go executable's path and exact bytes without executing it; approved execution checks its version before staging or user-home writes, then rechecks the bound executable under the Bootstrap lock before building. Unsupported host, missing prerequisite, malformed identity, unsafe source checkout, or unavailable exact commit fails during planning; an incompatible Go version fails only after approval but still before staging or user-home writes. Status, uninstall, prune, and their plans require neither the source checkout nor Git, `make`, Go, or source build/verification code; they execute only Bootstrap control-plane logic, whether invoked from the installed helper or an explicitly chosen checkout. Retention protocol calls require none of those prerequisites either.
...
For V1 removal, automatic recovery covers crashes at completed command boundaries where every surviving staged candidate still matches its recorded complete payload digest. A crash before the removal transaction is published may leave an unauthoritative staging file; a crash inside recursive deletion may leave a partial candidate. Neither path is inferred as owned or deleted automatically. Planning and execution fail closed, and the operator must inspect the residual paths and any published transaction before further recovery. The install/upgrade guarantee that both generation identities remain after a pointer switch does not apply to an approved prune or uninstall after physical deletion begins.
...
| AC-07 | Crash injection covers initial install, upgrade, uninstall, and prune at completed command boundaries around transaction, external-link, `current`/previous, generation-removal, and manifest changes. Recovery uses only recorded identities and exact manifest entries; unrelated operations fail closed, matching recovery requires a freshly approved plan, and `status` never repairs. A crash inside recursive deletion that leaves a partial candidate, or before removal transaction publication that leaves unauthoritative staging, blocks automatic recovery for manual inspection. |
