...
當時記錄為一個開放問題：一個不知道脈絡的讀者，看到「story 由 PraxisBound 管理」的定義
卻看到手寫的 story 檔案，很可能會「順手修正」它。

問題現在被回答了。原 ForgeFlowV2 位於 `/Users/carl/Dev/CMG/ForgeFlowV2`，並已更名為
...
也仍然只驗路徑、不解析內容。Story 內容的所有權留在 PraxisBound 那一側，
...
命名上，本 ADR 之後 ForgeFlowV2 與 PraxisBound 指同一個專案。`CONTEXT.md` 與文件中的措辭全面更新為 PraxisBound。
