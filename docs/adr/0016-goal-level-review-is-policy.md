...
Progression 與 final acceptance 必須分開。Goal completion readiness 是 ACTIVE、非空 Goal 上所有 Work Item VERIFIED、各自 latest PASS 仍匹配目前 Candidate、且沒有 OPEN Gate 的 fail-closed projection，並保留構成它的 Verification Evidence IDs；它本身仍不是持久化狀態。經 [ADR-0037](0037-goal-completion-has-no-human-final-review.md) 更新後，該 projection 由 typed action 以 exact aggregate evidence 原子完成 Goal，不設人工 Goal-final-review 選項。持久化 READY 同樣 fail closed：只有帶 current repository facts 的 transition 可用 VERIFIED promotion downstream，facts 在 state transaction 的受鎖 callback 內解析，缺少時就保留 PENDING；refresh 只觸及已改變 prerequisite 的 dependents 或被 unblock 的 Goal。
